  ...
  const missing = [];
      ...
      if (startDate && !missing.length) {
              ...
              if (new Date(startDate) < today) {
                        // A 4xx HERE COSTS THE CUSTOMER HIS ANSWER (11 septembre 2026).
                        //
                        // A step that returns 4xx stops the flow, and the customer gets
                        // nothing at all - no quote, no question, silence. That is the
                        // documented lesson of this project, and this branch was still
                        // breaking it: measured on the 26 January replay, the Quote flow
                        // reached generate-quote with everything it needed and died here.
                        //
                        // A past date is almost always a missing or mistyped year -
                        // someone writing in December about "15 February". That is a
                        // question, not a failure, and asking it costs nothing.
                        const guessed = new Date(startDate);
                        const nextYear = Number.isFinite(guessed.getTime())
                          ? new Date(Date.UTC(today.getUTCFullYear() + (guessed.getUTCMonth() < today.getUTCMonth() ? 1 : 0),
                                              guessed.getUTCMonth(), guessed.getUTCDate())).toISOString().slice(0, 10)
                          : '';
                        return askCustomer(res, {
                                  reason: 'START_DATE_IN_THE_PAST',
                                  question: 'we have read your rental as starting on ' + startDate +
                                            ', which is already past (today is ' + today.toISOString().slice(0, 10) +
                                            '). Could you confirm the season you mean' +
                                            (nextYear ? ' - did you mean ' + nextYear + '?' : '?'),
                                  startDate, endDate,
                        });
              }
      }
