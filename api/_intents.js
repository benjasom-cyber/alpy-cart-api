...
const D = {
  ...
  PARTIAL_CANCELLATION: {
    ...
    nl: [
      [/\b(?:helm|schoenen|skischoenen|stokken|ski's?|snowboard|materiaal|uitrusting|spullen) .{0,30}annuleren\b/, 3], [/\b(?:materiaal|uitrusting|spullen) van [a-z]+\b/, 2], [/\bannuleren van (?:de |een )?(?:helm|schoenen|stokken)\b/, 3],
  ...
  DEPOT_SWITCH: {
    en: [[/\b(?:ski )?(?:depot|storage|lockers?|store (?:the|our|my) skis?|overnight)\b/, 3], [/\b(?:swap|switch|exchange|change) (?:the |my |our )?(?:skis?|model|equipment)\b/, 3], [/\b(?:specific|particular|certain) (?:ski |snowboard )?(?:model|brand)\b/, 3], [/\b(?:specify|choose|request|pick|select) (?:the |a |which |what )?(?:ski |snowboard )?(?:model|brand|length)\b/, 3], [/\b(?:what|which) (?:ski |snowboard )?(?:model|brand) (?:and (?:length|size) )?(?:we|i) (?:want|get|would like|prefer)\b/, 3], [/\b(?:atomic|head|rossignol|volkl|salomon|stockli|nordica|fischer|k2|blizzard|elan|dynastar)\b/, 2], [/\b(?:length|size) \d{3} ?cm\b/, 2], [/\bmodel ?change\b/, 3], [/\b(?:leave|keep) (?:the |our |my )?skis? (?:at|in) the shop\b/, 3],
  ...
  PERSONAL_INFO: {
    en: [[/\b\d{2,3} ?(?:kg|kilos?)\b/, 2], [/\b\d{3} ?cm\b/, 1], [/\b(?:shoe|boot|foot) sizes?\b/, 3], [/\b(?:height|weight)s?\b/, 2], [/\b(?:missing|update|correct|change|wrong) (?:the )?(?:skier |personal |fitting )?(?:details|information|data|measurements)\b/, 3], [/\b(?:ski(?:ing)? )?(?:level|ability)\b/, 1], [/\bdate of birth\b/, 2], [/\bnames? of the skiers?\b/, 3], [/\bchange (?:the |a )?name\b/, 2], [/\bsize\b/, 1]],
  ...
  CHANGE_OF_SHOP: {
    en: [[/\b(?:move|transfer|change|switch|swap)\b[^.!?]{0,40}\bto (?:a |an |another |the )?(?:different |other |new |nearer |closer )?(?:shop|store|rental shop|location|resort|village|pick-?up (?:point|location))\b/, 3],
         ...
         [/\b(?:move|transfer) (?:the |my |our )?(?:booking|reservation) to (?!(?:the )?\d|(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|mon|tue|wed|thu|fri|sat|sun|next|another|a |the |one |two ))[a-z]/, 3], [/\b(?:change|move|switch) (?:the |my |our )?(?:pick-?up |collection |rental )?(?:location|place|point)\b/, 3], [/\b(?:a |the )?shop (?:in|at|near|closer to)\b/, 1], [/\bshop\b/, 1]],
    ...
    fr: [[/\b(?:deplacer|transferer|changer|basculer|transfert)\b[^.!?]{0,40}\b(?:vers|a|au|dans|pour) (?:un |une |le |la )?(?:autre |nouveau |nouvelle )?(?:magasin|boutique|station|loueur|point de retrait)\b/, 3],
         [/\btransfer\w+ (?:la |les |ma |notre )?reservations?\b/, 3], [/\btransfert\b/, 3], [/\bchang\w+ de magasin\b/, 3],
         [/\b(?:modifier|changer|deplacer) (?:le |de )?lieu (?:de (?:reservation|retrait|location|la location|prise en charge))?\b/, 3], [/\blieu de (?:retrait|reservation)\b/, 2], [/\b(?:autre|mauvais) magasin\b/, 3],
  ...
  PAYMENT: {
    ...
    fr: [[/\bpaiement (?:refuse|echoue|impossible|bloque|n'?a pas (?:fonctionne|abouti|marche)|probleme)\b/, 3], [/\bcarte (?:refusee|rejetee|debitee|bloquee)\b/, 3], [/\b(?:pas|toujours pas|jamais) (?:ete )?(?:debite|preleve|rembourse)\w*\b/, 3], [/\b(?:ou en est|j'?attends|quand (?:aurai|recevrai)) (?:le |mon |notre )?remboursement\b/, 3], [/\b(?:payer|regler) (?:le |la )?(?:solde|reste|acompte|restant)\b/, 3], [/\blien de paiement\b/, 3], [/\b(?:preleve|debite) (?:deux fois|en trop|un montant)\b/, 3], [/\bdifference entre le (?:prix|montant)\b/, 3], [/\brembourser (?:cette|la|moi la) difference\b/, 3], [/\b(?:different|differe) du montant\b/, 3], [/\bmontant (?:different|preleve|debite|facture)\b/, 2], [/\b(?:debit|prelevement) (?:en date )?du \d/, 2], [/\b(?:paiement|paye|prelevement)\b/, 1], [/\bremboursement\b/, 1]],
