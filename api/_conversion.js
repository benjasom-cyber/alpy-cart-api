 ...
 *
 * MESURE DE CONTRÔLE (1er-15 août 2026, morte-saison, faite à la main) :
 * 200 tickets, 166 demandeurs avec une adresse, 11 connus de la base
 * réservations. 157 contacts d'avant-vente → 2 conversions, 889,60 € ;
 * 3 réachats après contact → 2 551,85 €. Le mécanisme est prouvé ; le chiffre,
 * lui, n'aura de sens qu'en saison.
 *
 * LE TÉLÉPHONE
 *
 ...
 *
 *   METABASE_URL       ex. https://metabase.alpy.com   (sans slash final)
 *   METABASE_API_KEY   une clé d'API Metabase avec accès à la base BigQuery
 *   METABASE_DB_ID     l'id de la base dans Metabase (2 sur cette instance)
 *
 * Sans ces variables l'endpoint répond 200 avec `ok:false` et le nom de ce qui
 * manque — jamais une erreur muette.
 */
...
const MB_URL = String(process.env.METABASE_URL || '').trim().replace(/\/+$/, '');
...
export async function handler(req, res) {
  ...
  if (!MB_KEY) missing.push('METABASE_API_KEY');
  if (missing.length) {
  ...
  try {
    ...
    let rows = [];
    if (entries.length) {
      // BigQuery encaisse largement quelques milliers de STRUCT ; au-delà on
      // découpe pour ne pas fabriquer une requête d'un mégaoctet.
      const CHUNK = 2000;
    ...
    if (!truncated) CACHE.set(key, { at: Date.now(), value });
