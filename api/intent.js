...
export default async function handler(req, res) {
      ...
      // A SHOP CHANGE is the model's blind spot (582145: "move our booking to a
      // shop in Sauze d'Oulx" came back DEPOT_SWITCH, and the Shop services flow
      // answered about lockers). When the dictionary reads a shop change clearly,
      // it goes to General questions, which rebuilds the cart at the new shop
      // (D-56) - unless the model saw a cancellation or a date change instead.
      const dictShopChange = (dictRes.scores || []).find(x => x.topic === 'CHANGE_OF_SHOP');
      const shopChangeWins = dictShopChange && dictShopChange.score >= 3 &&
        (!llm || ['DEPOT_SWITCH', 'GENERAL_QUESTION', 'OTHER', 'QUOTE', 'REQUOTE'].includes(llm.topic));

      let decision;
      if (kw) decision = kw;
      else if (!pend && !off && shopChangeWins) decision = { topic: 'GENERAL_QUESTION', source: 'dictionary_shop_change', blocked: false };
